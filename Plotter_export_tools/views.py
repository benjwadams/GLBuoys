...
def getTimeIndices(loc_id, loc_alias, date_start, date_end):
    ...
    try:
        ...
        t_ser = pd.Series(ds['time'][:])

        try:
            t_ser_slice = t_ser[(t_ser >= lst_dsec[0]) &
                                (t_ser <= lst_dsec[-1])]
            min_idx = t_ser_slice.idxmin()
            max_idx = t_ser_slice.idxmax()
            lst_idx = [min_idx, max_idx]
        except ValueError:
            # TODO: add logging if we can't find min/max
            lst_idx = [-9999, -9999]
